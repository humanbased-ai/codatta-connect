import { h as xt, i as Tt, a as at, b as st, c as F, H as Jt, d as te, t as ee, e as ne, f as qt, g as re, r as oe, s as ie } from "./main-D60MaR66.js";
...
function de(e) {
  ...
  return function(l, g) {
    ...
    if (Ht(l, g) !== 1)
      throw new Error("Cannot find square root");
    let c = t, E = l.mul(l.ONE, i), x = l.pow(g, n), S = l.pow(g, o);
    for (; !l.eql(x, l.ONE); ) {
      if (l.is0(x))
        return l.ZERO;
      let _ = 1, N = l.sqr(x);
      for (; !l.eql(N, l.ONE); )
        if (_++, N = l.sqr(N), _ === c)
          throw new Error("Cannot find square root");
      const L = k << BigInt(c - _ - 1), U = l.pow(E, L);
      c = _, E = l.sqr(U), x = l.mul(x, E), S = l.mul(S, U);
    }
    return S;
  };
}
...
function wt(e, n, t = !1, r = {}) {
  ...
  if (typeof n == "object" && n != null) {
    ...
    const c = n;
    c.BITS && (s = c.BITS), c.sqrt && (i = c.sqrt), typeof c.isLE == "boolean" && (t = c.isLE);
  } else
  ...
  const g = Object.freeze({
    ...
    mulN: (c, E) => c * E,
    inv: (c) => _t(c, e),
    sqrt: i || ((c) => (l || (l = he(e)), l(g, c))),
    toBytes: (c) => t ? Ut(c, a) : It(c, a),
    fromBytes: (c) => {
      if (c.length !== a)
...
function Ot(e, n, t) {
  ...
  a > r && (a -= i, l += X);
  const g = n * r, c = g + Math.abs(a) - 1, E = a === 0, x = a < 0, S = n % 2 !== 0;
  return { nextN: l, offset: c, isZero: E, isNeg: x, isNegF: S, offsetF: g };
}
function Ee(e, n) {
  if (!Array.isArray(e))
  ...
  return {
    ...
    wNAF(t, r, s) {
      ...
      for (let l = 0; l < a.windows; l++) {
        const { nextN: g, offset: c, isZero: E, isNeg: x, isNegF: S, offsetF: _ } = Ot(s, l, a);
        s = g, E ? o = o.add(rt(S, r[_])) : i = i.add(rt(x, r[c]));
      }
      return Zt(s), { p: i, f: o };
    },
    /**
    ...
    wNAFUnsafe(t, r, s, i = e.ZERO) {
      ...
      for (let a = 0; a < o.windows && s !== et; a++) {
        const { nextN: l, offset: g, isZero: c, isNeg: E } = Ot(s, a, o);
        if (s = l, !c) {
          const x = r[g];
...
function Se(e, n, t, r) {
  ...
  const o = e.ZERO, a = fe(BigInt(s));
  let l = 1;
  a > 12 ? l = a - 3 : a > 4 ? l = a - 2 : a > 0 && (l = 2);
  const g = ht(l), c = new Array(Number(g) + 1).fill(o), E = Math.floor((n.BITS - 1) / l) * l;
  let x = o;
  for (let S = E; S >= 0; S -= l) {
    c.fill(o);
    for (let N = 0; N < i; N++) {
    ...
    let _ = o;
    for (let N = c.length - 1, L = o; N > 0; N--)
      L = L.add(c[N]), _ = _.add(L);
    if (x = x.add(_), S !== 0)
      for (let N = 0; N < l; N++)
        x = x.double();
  }
...
const V = {
  ...
  _tlv: {
    encode: (e, n) => {
      ...
      const r = n.length / 2, s = ct(r);
      if (s.length / 2 & 128)
        throw new t("tlv.encode: long form length too big");
      const i = r > 127 ? ct(s.length / 2 | 128) : "";
      return ct(e) + i + s + n;
    },
  ...
  toSig(e) {
    ...
    const { v: a, l } = r.decode(2, i), { v: g, l: c } = r.decode(2, l);
    if (c.length)
      throw new n("invalid signature: left bytes after parsing");
...
}, ot = BigInt(0), it = BigInt(1), qe = BigInt(2), ft = BigInt(3), Ae = BigInt(4);
function _e(e, n, t) {
  function r(s) {
...
function He(e, n = {}) {
  ...
  function l(H, f, h) {
    const { x: u, y: d } = f.toAffine(), w = t.toBytes(u);
  ...
  function g(H) {
    at(H);
    const f = t.BYTES, h = f + 1, u = 2 * f + 1, d = H.length, w = H[0], p = H.subarray(1);
    if (d === h && (w === 2 || w === 3)) {
      ...
      const m = t.fromBytes(p.subarray(f * 0, f * 1)), y = t.fromBytes(p.subarray(f * 1, f * 2));
      if (!S(m, y))
  ...
  const c = n.toBytes || l, E = n.fromBytes || g, x = _e(t, e.a, e.b);
  function S(H, f) {
    const h = t.sqr(f), u = x(H);
    return t.eql(h, u);
  }
  if (!S(e.Gx, e.Gy))
    throw new Error("bad curve params: generator point");
  const _ = t.mul(t.pow(e.a, ft), Ae), N = t.mul(t.sqr(e.b), BigInt(27));
  if (t.is0(t.add(_, N)))
    throw new Error("bad curve params: a or b");
  function L(H, f, h = !1) {
    if (!t.isValid(f) || h && t.is0(f))
  ...
  const W = At((H, f) => {
    ...
    if (t.eql(d, t.ONE))
      return { x: h, y: u };
    const w = H.is0();
    f == null && (f = w ? t.ONE : t.inv(d));
    const p = t.mul(h, f), m = t.mul(u, f), y = t.mul(d, f);
    if (w)
  ...
  function Q(H, f, h, u, d) {
    return h = new I(t.mul(h.px, H), h.py, h.pz), f = rt(u, f), h = rt(d, h), f.add(h);
  }
  class I {
    /** Does NOT validate if the point is valid. Use `.assertValidity()`. */
    constructor(f, h, u) {
      this.px = L("x", f), this.py = L("y", h, !0), this.pz = L("z", u), Object.freeze(this);
    }
    /** Does NOT validate if the point is valid. Use `.assertValidity()`. */
    static fromAffine(f) {
      const { x: h, y: u } = f || {};
      if (!f || !t.isValid(h) || !t.isValid(u))
     ...
     */
    precompute(f = 8, h = !0) {
      return K.setWindowSize(this, f), h || this.multiply(ft), this;
    }
    ...
    equals(f) {
      U(f);
      const { px: h, py: u, pz: d } = this, { px: w, py: p, pz: m } = f, y = t.eql(t.mul(h, m), t.mul(w, d)), B = t.eql(t.mul(u, m), t.mul(p, d));
      return y && B;
    }
    ...
    double() {
      const { a: f, b: h } = e, u = t.mul(h, ft), { px: d, py: w, pz: p } = this;
      let m = t.ZERO, y = t.ZERO, B = t.ZERO, b = t.mul(d, d), O = t.mul(w, w), q = t.mul(p, p), v = t.mul(d, w);
      return v = t.add(v, v), B = t.mul(d, p), B = t.add(B, B), m = t.mul(f, B), y = t.mul(u, q), y = t.add(m, y), m = t.sub(O, y), y = t.add(O, y), y = t.mul(m, y), m = t.mul(v, m), B = t.mul(u, B), q = t.mul(f, q), v = t.sub(b, q), v = t.mul(f, v), v = t.add(v, B), B = t.add(b, b), b = t.add(B, b), b = t.add(b, q), b = t.mul(b, v), y = t.add(y, b), q = t.mul(w, p), q = t.add(q, q), b = t.mul(q, v), m = t.sub(m, b), B = t.mul(q, O), B = t.add(B, B), B = t.add(B, B), new I(m, y, B);
    }
    ...
    add(f) {
      U(f);
      const { px: h, py: u, pz: d } = this, { px: w, py: p, pz: m } = f;
      let y = t.ZERO, B = t.ZERO, b = t.ZERO;
      const O = e.a, q = t.mul(e.b, ft);
    ...
    multiply(f) {
      ...
      if (h) {
        const { k1neg: p, k1: m, k2neg: y, k2: B } = h.splitScalar(f), { p: b, f: O } = w(m), { p: q, f: v } = w(B);
    ...
    multiplyUnsafe(f) {
      ...
      if (h) {
        const { k1neg: d, k1: w, k2neg: p, k2: m } = h.splitScalar(f), { p1: y, p2: B } = xe(I, u, w, m);
        return Q(h.beta, y, B, d, p);
      } else
        return K.wNAFCachedUnsafe(u, f);
    }
    multiplyAndAddUnsafe(f, h, u) {
      const d = this.multiplyUnsafe(h).add(f.multiplyUnsafe(u));
     ...
     */
    isTorsionFree() {
      const { isTorsionFree: f } = n;
      return s === it ? !0 : f ? f(I, this) : K.wNAFCachedUnsafe(this, i).is0();
    }
    clearCofactor() {
      const { clearCofactor: f } = n;
      return s === it ? this : f ? f(I, this) : this.multiplyUnsafe(s);
    }
    toBytes(f = !0) {
      return lt("isCompressed", f), this.assertValidity(), c(I, this, f);
    }
...
function Oe(e, n, t = {}) {
  ...
  function I(u) {
    return ce("num < 2^" + l, u, ot, Q), o.toBytes(u);
  }
  function D(u, d, w = K) {
    ...
    function R(z) {
      ...
      return m && g(G) && (tt = c(G), gt ^= 1), new x(J, tt, gt);
    }
...
function je(e) {
  const n = ut.p, t = BigInt(3), r = BigInt(6), s = BigInt(11), i = BigInt(22), o = BigInt(23), a = BigInt(44), l = BigInt(88), g = e * e * e % n, c = g * g * e % n, E = M(c, t, n) * c % n, x = M(E, t, n) * c % n, S = M(x, Bt, n) * g % n, _ = M(S, s, n) * S % n, N = M(_, i, n) * _ % n, L = M(N, a, n) * N % n, U = M(L, l, n) * L % n, W = M(U, a, n) * N % n, Y = M(W, t, n) * c % n, Q = M(Y, o, n) * _ % n, I = M(Q, r, n) * g % n, D = M(I, Bt, n);
...
const vt = wt(ut.p, void 0, void 0, { sqrt: je }), Me = Te({
  ...
  endo: {
    ...
    splitScalar: (e) => {
      ...
      let g = C(e - a * t - l * s, n), c = C(-a * r - l * i, n);
      const E = g > o, x = c > o;
      if (E && (g = n - g), x && (c = n - c), g > o || c > o)
        throw new Error("splitScalar: Endomorphism failed, k=" + e);
      return { k1neg: E, k1: g, k2neg: x, k2: c };
    }
  }
}, ie);
export {
